...
export const SqordinalSqore = ({ track, set, setCanvas, setSqord, setSet }: any) => {
  const mySet: any = useRef();
  const myHash: any = useRef();
  ...
  const hash = pseudoHash(`${songNames[track]}${songNames[track]}`);

  useEffect(() => {
    if ((!_.isUndefined(mySet.current) && mySet.current !== set) || (!_.isUndefined(myHash.current) && myHash.current !== hash)) {
      myHash.current = hash;
      window.set = 0;
      myGroup.current.remove();
      mySqord.current = makeSqord(hash, false, null);
  ...
  useEffect(() => {
    if (!mySqord.current) {
      ...
      const animate = () => {
        ...
        // illo.rotate.y = 1.1178174815425923;
        // illo.rotate.x = -0.08965265493642748;
        // illo.rotate.z = 0;
